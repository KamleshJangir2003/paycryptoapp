...
<div class="row g-3 mb-4">
    <div class="col-6">
        <a href="https://wa.me/message/SEAK4ON7KSOEP1" target="_blank" class="card p-2 text-decoration-none d-flex flex-column align-items-center justify-content-center text-center" style="min-height:90px;">
...
<div class="card">
    ...
    <div class="card-body p-0">
        ...
        <div class="d-md-none">
            ...
            <div style="padding:12px 14px;border-bottom:1px solid #1e1e3a;">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <span class="badge bg-secondary" style="font-size:.72rem;">{{ ucfirst($tx->type) }}</span>
                        <span style="color:#8888aa;font-size:.72rem;margin-left:6px;">{{ ucfirst($tx->wallet) }}</span>
                    </div>
                    <span class="badge badge-{{ $tx->status }}" style="font-size:.7rem;">{{ ucfirst($tx->status) }}</span>
                </div>
                <div class="d-flex justify-content-between align-items-end mt-1">
                    <div>
                        <div class="{{ $tx->direction === 'credit' ? 'text-green' : 'text-red' }}" style="font-weight:700;font-size:1rem;">
                            {{ $tx->direction === 'credit' ? '+' : '-' }}₹{{ number_format($tx->amount, 2) }}
                        </div>
                        @if($tx->type === 'deposit' && $tx->deposit && $tx->deposit->payment_type === 'usdt')
                        <div style="color:#26a17b;font-size:.72rem;font-weight:500;">{{ number_format($tx->deposit->usdt_amount, 4) }} USDT</div>
                        @endif
                    </div>
                    <div style="color:#8888aa;font-size:.72rem;">{{ $tx->created_at->format('d M, h:i A') }}</div>
                </div>
            </div>
            @empty
...
<a href="https://wa.me/message/SEAK4ON7KSOEP1" target="_blank"
   style="position:fixed;bottom:140px;right:18px;z-index:9999;display:flex;flex-direction:column;align-items:center;gap:4px;text-decoration:none;">
    <div style="width:40px;height:40px;background:#25D366;border-radius:50%;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(37,211,102,.5);">
    ...
    <div style="color:#25D366;font-size:.68rem;font-weight:700;text-shadow:0 1px 4px rgba(0,0,0,.8);">Help Chat</div>
</a>

<a href="https://t.me/YOUR_TELEGRAM_GROUP_LINK" target="_blank"
   style="position:fixed;bottom:80px;right:18px;z-index:9999;display:flex;flex-direction:column;align-items:center;gap:4px;text-decoration:none;">
    <div style="width:40px;height:40px;background:#0088cc;border-radius:50%;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(0,136,204,.5);">
    ...
    <div style="color:#0088cc;font-size:.68rem;font-weight:700;text-shadow:0 1px 4px rgba(0,0,0,.8);">Telegram</div>
</a>
